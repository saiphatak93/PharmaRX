...
// ─── EmailJS (same config as AdminLogin.jsx) ──────────────────────────────────
const EMAILJS_SERVICE_ID  = "YOUR_SERVICE_ID";
const EMAILJS_TEMPLATE_ID = "YOUR_TEMPLATE_ID";   // can use same or a separate welcome template
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rx-auth-form-sub { color: #64748b; font-size: 14px; margin-bottom: 28px; }
  ...
  .rx-auth-input.error { border-color: #ef4444; }
  .rx-auth-group { margin-bottom: 16px; }

  .rx-auth-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }

  /* scopes eye button to the input height only, not the full group */
  .rx-auth-input-wrap { position: relative; display: block; }

  .rx-auth-eye {
    position: absolute; right: 12px; top: 50%; transform: translateY(-50%);
    background: none; border: none; cursor: pointer;
    font-size: 16px; color: #94a3b8; padding: 4px; line-height: 1;
    z-index: 2;
  }

  .rx-auth-strength { display: flex; gap: 4px; margin-top: 6px; }
  .rx-auth-strength-bar { flex: 1; height: 3px; border-radius: 99px; background: #e2e8f0; transition: background 0.3s; }
  .rx-auth-strength-bar.weak   { background: #ef4444; }
  .rx-auth-strength-bar.fair   { background: #f59e0b; }
  .rx-auth-strength-bar.good   { background: #10b981; }
  .rx-auth-strength-label { font-size: 11px; color: #64748b; margin-top: 4px; font-weight: 600; }
  ...
  .rx-auth-footer { text-align: center; color: #64748b; font-size: 13.5px; margin-top: 16px; }
  ...
  .rx-auth-notify-badge {
    ...
    padding: 4px 10px; border-radius: 99px; margin-bottom: 22px;
  }
  ...
  /* Register-specific: password row single column on mobile */
  @media (max-width: 600px) {
    .rx-auth-row { grid-template-columns: 1fr; gap: 0; }
  }
...
const STRENGTH_COLORS = ["", "weak", "fair", "good", "good"];
...
export default function AdminRegister() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      sendWelcomeEmail(values.email, values.name);
      setSuccess(`✅ Account created! Welcome, ${values.name}. Redirecting…`);
      ...
      <div className="rx-auth-page">
        ...
        <div className="rx-auth-left">
          ...
          <div className="rx-auth-hero">
            <h1>Join PharmaRx Today</h1>
            <p>Set up your pharmacy account in minutes and start managing your inventory the smart way.</p>
          </div>

          <div className="rx-auth-features">
            {[
              { icon:"🚀", text:"Get started in under 2 minutes"               },
              { icon:"📧", text:"Receive a welcome email upon registration"     },
              { icon:"🔐", text:"Secure, encrypted authentication via Firebase" },
            ].map(f => (
        ...
        <div className="rx-auth-right">
          <div className="rx-auth-form-wrap">
            ...
            <div className="rx-auth-form-title">Create account</div>
            <div className="rx-auth-form-sub">Fill in the details below to get started</div>

            {/* Full name */}
            <div className="rx-auth-group">
              <label className="rx-auth-label">Full Name</label>
              <input
                type="text"
            ...
            <div className="rx-auth-row">
              <div className="rx-auth-group">
                ...
                />
                <button className="rx-auth-eye" type="button" onClick={() => setShowPwd(v => !v)}>
                  {showPwd ? "🙈" : "👁️"}
                </button>
              </div>
                {values.password && (
                  <>
                    <div className="rx-auth-strength">
                      {[1,2,3,4].map(i => (
                        <div key={i} className={`rx-auth-strength-bar${i <= strength ? " " + strengthColor : ""}`} />
                      ))}
                    </div>
                    <div className="rx-auth-strength-label">{strengthLabel}</div>
                  </>
                )}
              </div>

              <div className="rx-auth-group">
                <label className="rx-auth-label">Confirm Password</label>
                <div className="rx-auth-input-wrap">
                <input
                  type={showCfm ? "text" : "password"}
                  className={`rx-auth-input${values.confirm && values.confirm !== values.password ? " error" : ""}`}
                  placeholder="••••••••"
                  style={{ paddingRight: 40 }}
                  value={values.confirm}
                  onChange={e => setValues(p => ({ ...p, confirm: e.target.value }))}
                  onKeyDown={e => e.key === "Enter" && handleRegister()}
                />
                <button className="rx-auth-eye" type="button" onClick={() => setShowCfm(v => !v)}>
                  {showCfm ? "🙈" : "👁️"}
                </button>
              </div>
            ...
            {error   && <div className="rx-auth-error">⚠️ {error}</div>}
            {success && <div className="rx-auth-success">{success}</div>}

            <button className="rx-auth-btn" onClick={handleRegister} disabled={loading}>
              {loading ? <><div className="rx-spinner" /> Creating account…</> : "Create Account →"}
            </button>
